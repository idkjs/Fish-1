...
let () = {
  let board =
    ...
    |> Board.create;
  let players = [Player_color.Black, Player_color.White];
  let pos11 = {Position.row: 1, col: 1};
  let pos31 = {Position.row: 3, col: 1};
  let state = Game_state.create(board, players);
  let state = Game_state.place_penguin(state, Player_color.Black, pos11);
  let state = Game_state.place_penguin(state, Player_color.White, pos31);
  Render.render(state);
