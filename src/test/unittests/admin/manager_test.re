...
let fast_referee_timeout_config = {
...
let fast_timeout_config = {
  Manager.inform_tournament_start_timeout_ms: 100,
  inform_tournament_result_timeout_ms: 100,
};

let shuffle = (xs: list('a)): list('a) => {
  let padded = List.map(~f=x => (Random.bits(), x), xs);
  let sorted =
    List.sort(~compare=((n1, _), (n2, _)) => Int.compare(n1, n2), padded);
    ...
    [
      OUnit2.(>::)("test_run_tournament_mix_types_of_players", _ =>{
        let failed = [
          ...
          MP.get_player_hang_at_color_assignment_and_disqualification(
            "failed-4",
          ),
          MP.get_player_hang_at_color_assignment_and_disqualification(
            "failed-5",
        ...
        let others = List.init(10, ~f=Util.get_default_ai_player);
        /* ages are the same, so order is based on initial order */
        let players = failed @ cheaters @ others |> shuffle;
        let result =
        ...
        Util.check_same_set_of_players_by_names(
          failed,
          result.all_failed_players,
        );
        Util.check_same_set_of_players_by_names(
          others,
          result.final_winners @ result.all_losers,
        );
      }),
    ],
  );
