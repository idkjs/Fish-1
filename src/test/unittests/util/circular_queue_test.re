  ...
  OUnit2.(>:::)(
    ...
    [
      ...
      OUnit2.(>::)("test_remove_current", _ => {
        ...
        let int_cq213 = CQ.create(2, [1, 3]);
        let int_cq13 = CQ.remove_current(int_cq213);
        OUnit.assert_equal(Some([1, 3])) @@ Option.map(CQ.to_list, int_cq13);
        OUnit.assert_equal(Some(1)) @@ Option.map(CQ.get_current, int_cq13);
        OUnit.assert_equal(
          Some([3, 1]),
